...
class NCF:
    ...
    def _create_model(self,):
        ...
        with tf.compat.v1.variable_scope("input_data", reuse=tf.compat.v1.AUTO_REUSE):
            ...
            self.user_input = tf.compat.v1.placeholder(tf.int32, [None, self.input_size], name='user_input')
            self.item_input = tf.compat.v1.placeholder(tf.int32, [None, self.input_size], name='item_input')
            self.labels = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])

        with tf.variable_scope("embedding", reuse=tf.compat.v1.AUTO_REUSE):
        ...
        with tf.variable_scope("gmf", reuse=tf.compat.v1.AUTO_REUSE):
        ...
        with tf.variable_scope("mlp", reuse=tf.compat.v1.AUTO_REUSE):
        ...
        with tf.variable_scope("ncf", reuse=tf.compat.v1.AUTO_REUSE):
    ...
    def fit(self, data):
        ...
        for epoch_count in range(1, self.n_epochs + 1):
            ...
            if self.verbose and epoch_count % self.verbose == 0:
                ...
                print("Epoch %d [%.2fs]: train_loss = %.6f " % (epoch_count, train_time, sum(train_loss) / len(train_loss)))
