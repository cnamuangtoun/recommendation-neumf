...
def parse_args():
    parser = argparse.ArgumentParser(description='RecommenToTopKUser')
    ...
    parser.add_argument('--topK', type=int, default=100, help='amount of users to recommended per item')
...
if __name__ == '__main__':
    ...
    topKpredictions = df_result.groupby('ActivityID')['UserID','Score'].apply(lambda x: x.nlargest(topK,columns=['Score'])).reset_index()
    topKpredictions.drop(columns=[topKpredictions.columns[1]],inplace=True)
    topKpredictions.to_csv(os.path.join(dir,output_subdir,"Recommend_topK_user_" + datetime + ".csv"),index=False)
